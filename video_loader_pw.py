...
class VideoLoaderPW:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "align_frames": (["none", "8n+1", "4n+1"], {"default": "8n+1", "tooltip": "Align generate segment frames by adjusting split points or repeating end frames. 'none' disables alignment."}),
    ...
    def load_video(self, video, frame_rate, display_mode, start_time, end_time, start_frame, end_frame, crop_x=0.0, crop_y=0.0, crop_w=1.0, crop_h=1.0, split_count=0, split_purple_point=0.0, split_purple_point_idx=0, split_green_point=0.0, split_green_point_idx=0, select_generate="blue", align_frames="8n+1", path=None, **kwargs):
        video_to_load = path.strip() if (path and isinstance(path, str) and path.strip()) else video

        # 计算对齐除数：none -> 0, 4n+1 -> 4, 8n+1 -> 8
        if align_frames == "8n+1":
            align_divisor = 8
        elif align_frames == "4n+1":
            align_divisor = 4
        else:
            align_divisor = 0
        ...
        if split_count == 0:
            ...
            if align_divisor > 0 and (total_frames - 1) % align_divisor != 0:
                new_total_frames = math.ceil((total_frames - 1) / align_divisor) * align_divisor + 1
            ...
            if align_divisor > 0:
                if not select_gen:
                    ...
                    target_N = math.ceil((N - 1) / align_divisor) * align_divisor + 1
