    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "VideoLoaderPW") {
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                // 【核心重构】：利用 ComfyUI 局部执行机制
                const loadReloadVideo = async () => {
                    // 1. 清除所有缓存和 UI 状态
                    resetAllParams();
                    
                    // 2. 收集当前节点的所有 Widget 值
                    const inputs = {};
                    if (node.widgets) {
                    ...
                    // 3. 获取 path 的值 (兼容 Widget 和 Slot 连线)
                    let pathValue = inputs["path"] || "";
                    if (!pathValue && node.inputs) {
                        ...
                        if (pathInput && pathInput.link) {
                            ...
                            if (link) {
                                ...
                                if (originNode && originNode.widgets) {
                                    // 极简取值：直接读取上游节点的 path/video 字符串 widget
                    ...
                    if (!pathValue) {
                        console.warn("[VideoLoaderPW] No path found for local execution.");
                        return;
                    }
                    inputs["path"] = pathValue;

                    // 4. 构造局部 Prompt (只包含当前节点，实现秒级执行)
                    const nodeId = String(node.id);
                    const localPrompt = {
                        [nodeId]: {
                            "inputs": inputs,
                            "class_type": "VideoLoaderPW",
                            "_meta": { "title": node.title || "Video Loader PW" }
                        }
                    };
                    ...
                    try {
                        ...
                        if (!resp.ok) {
                            console.error("[VideoLoaderPW] Local execution failed:", await resp.text());
                        }
                    } catch (e) {
