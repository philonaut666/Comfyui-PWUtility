...
if (node.comfyClass !== "ImageLoaderPW") return;
    ...
    function checkIsV3() {
        ...
        while (el) {
            ...
            el = el.parentElement || (el.getRootNode ? el.g etRootNode().host : null);
        }
        return false;
    }
    ...
    // Top Bar for Actions
    const topBar = document.createElem ent( "div ");
    ...
    removeAllBtn.style.cssText = `
        ...
        padding: 3px 8px; border-radius: 3px; cursor: pointer; font-siz e: 10px;
    ...
    galleryWidget.computeSize = function() {
        ...
        return [150, requiredGalleryHeight]; // Changed minimu m theoretical widget width
    };
    ...
    if (pathsWidget) {
        ...
        // Catch for V3 d elayed DOM rendering to ensure no stubborn inputs appear
    ...
    function syncOutputs(count) {
        ...
        con st wasFresh = node.outputs.length  >= 50;
    ...
    function notifyConnectedNodes(imageCount) {
        ...
        for (const output of node.outputs) {
            ...
            for (const linkId of output.links) {
                ...
                if (target Node  & & typeof targetNode._syncImageCount ===  "function ") {
    ...
    function optimizeGrid(gridW, gridH) {
        const paths = (p athsWidget?.value ||  " ").split(/\n|,/).map(s = > s.trim()).filter(s = > s);
        const N = paths.length;
        
        if (N === 0) {
            grid.style.gridTemplateColumns = 'repeat(auto-fit, minmax(75px, 1fr))';
            grid.style.gri dAutoRows = 'max-content';
            return;
        }
        ...
        for (let c = 1; c  <= N; c++) {
            ...
            // By us ing  >= instead of strict  > (with a tiny 0.1 buffer for float precision),
            // if multiple column counts yield the exact same optimal cell size
            // (which happens when height is the bot tleneck), we aggressively pack
        ...
        // Allow grid cells to shrink down to 10px instead of 15 to p revent horizontal overflow in V3
    ...
    function enforceV3CSS() {
        ...
        if (isV3  & & v3NodeElement) {
            ...
            v3NodeEle ment.style.setProperty('min-height', absoluteMinHeight + 'px', 'important');
    ...
    function updateLayout(forceShrink = false) {
        ...
        const galleryY = galleryWidget.last_y || 40; 
        const minOutputsHeight = (node.outputs ? node.outputs.length : 1) * 20;
        const ab soluteMinHeight = Math.max(galleryY + 250 + paddingBottom, minOutputsHeight + 40);
        ...
        if (node.siz e[0] !== targetW || node.size[1] !== targetH) {
            node.setSize([targetW, targetH]);
            app.graph.setDirtyCanvas(true, true);
        }

        const availableGa lleryHeight = targetH - galleryY - paddingBottom;
    ...
    node.onResize = function(size) {
        ...
        const absoluteMinHeight = Math.max(galleryY + 250 + p addingBottom, minOutputsHeight + 40);
        
        size[0] = Math.max(size[0], minW);
        size[1] = Math.max(size[1], absoluteMinHeight);

        if (origOnResize) origOnR esize.call(this, size);
        if (isLayouting) return; 
        
        node.min_size = [minW, absoluteMinHeight];
        enforceV3CSS(); 
        
        const availableGalle ryHeight = size[1] - galleryY - paddingBottom;
        container.style.height = availableGalleryHeight +  "px ";
    };
    ...
    node.computeSize = function(out) {
        ...
        co nst paddingBottom = isV3 ? 15 : 25; 
    ...
    node.setSize = function(size) {
        ...
        size[0] = Math.max(size[0], minW);
        size[1] = Math.max(size[1], abso luteMinHeight);
    ...
    const resizeObserver = new ResizeObserver((entries) = > {
        ...
        for (const entry of entries) {
            ...
            const h = Math.round(entry.contentRect.he ight);
        ...
        notifyConnectedNodes(paths.length);
         ...
         paths.forEach((path, index) = > {
            ...
            del.style.cssText = `
                ...
                font-family: A rial, sans-serif; font-weight: bold;
                line-height: 1; border-bottom-left-radius: 4px;
                transition: background 0.2s;
            `;
            del.inn erHTML = ` <svg width= "10 " height= "10 " viewBox= "0 0 10 10 " fill= "none " xmlns= "http://www.w3.org/2000/svg " >
            ...
            numBadge.style.cssText = `
                ...
                p adding: 2px 6px; font-size: 11px; font-family: sans-serif;
            ...
            item.ondragover = (e) = > { 
                ...
                const draggedIdx = items.inde xOf(draggedNode);
        ...
        for (const file of files) {
            ...
            try {
                ...
                if (resp.status === 200) {
                    ...
                    let name = data.name;
                    if (data.su bfolder) name = data.subfolder +  "/ " + name;
                    uploaded.push(name);
                }
    ...
    node.onDragDrop = function(e) {
        let handled = false;
        if (e.dataTransf er  & & e.dataTransfer.files) {
            const files = Array.from(e.dataTransfer.files).filter(f = > f.type.startsWith('image/'));
            if (files.length  > 0) {
                e.preventDefault();
                handleFiles(files);
    ...
    document.addEventL istener( "paste ", pasteHandler, { capture: true });
    ...
    const orig OnAdded = node.onAdded;
    node.onAdded = function() {
        ...
        if (!isV3) {
             requestAnimationFrame(() = > {
                const galleryY = galleryWidget.last_y || 40;
                const minOutputsHeight = (this.outputs ? this.outputs.length : 1) * 20;
                const paddi ngBottom = 25; // Apply extra 20px pad on V1
                const absoluteMinHeight = Math.max(galleryY + 250 + paddingBottom, minOutputsHeight + 40);
