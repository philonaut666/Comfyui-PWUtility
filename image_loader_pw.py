...
def resize_image(self, image, width, height, resize_method= "keep proportion ", interpolation= "nearest ", multiple_of=0):
    ...
    if resize_method == 'keep proportion' or resize_method == 'pad':
        ...
        if resize_meth od == 'pad':
            pad_left = (width - new_width) // 2
            pad_right = width - new_width - pad_left
            pad_top = (height - new_height) // 2
            pad_b ottom = height - new_height - pad_top
    ...
    if resize_method == 'crop':
        i f x  > 0 or y  > 0 or x2  > 0 or y2  > 0:
            outputs = outputs[:, y:y2, x:x2, :]

    if multiple_of  > 1 and (outputs.shape[2] % multiple_of != 0 or outputs.shape[1] % multiple_of != 0):
        width = outputs.shape[2]
        height = outputs.shape[1]
        x = (width % multip le_of) // 2
...
def load_images(self, image_paths, width, height, interpolation, resize_method, multiple_of, img_ compression):
    results = []
    ...
    for path in valid_paths:
        try:
            ...
            image_tensor = torch.fr om_numpy(image_np)[None,]
    ...
    if len(results)  > 0:
        ...
        if all_same_shape:
            ...
            print( "MultiImageLoader Warning: Images have different dimensions due to resize settings. Cannot batch into multi_output. Outputting zero tensor for the batch, but individual output node s will still work fine. ")
            multi_output = torch.zeros((1, 64, 64, 3))
    else:
        # Fallback empty tensor if no valid paths
        multi_output = torch.zeros((1, 64, 64, 3))
        res ults = [multi_output]
