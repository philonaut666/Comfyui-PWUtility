...
class VideoSplitterPW:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "images": ("IMAGE",),
                "fps": ("FLOAT", {"default": 24.000, "min": 0.001, "max": 1000.0, "step": 0.001, "tooltip": "FPS used for frame-to-time conversion to align audio."}),
                "split_count": ("INT", {"default": 0, "min": 0, "max": 2, "step": 1, "tooltip": "0: No split, 1: Front only, 2: Front & Back"}),
                "align_frames": (["none", "MH3-17n+5", "LTX2.3-8n+1"], {"default": "MH3-17n+5", "tooltip": "Align split_generate frames to specific pattern."}),
    ...
    def split_video(self, images, fps, split_count, align_frames, split_front_point_idx, split_back_point_idx, audio=None, split_info=None):
        ...
        if use_info and info_has_split:
            ...
            # 【对齐帧数逻辑】仅在手动切分且 align_frames 不为 none 时生效
            if align_frames != "none" and split_count > 0:
                gen_frames = gen_end - gen_start + 1

                if align_frames == "LTX2.3-8n+1":
                ...
                elif align_frames == "MH3-17n+5":
                    # 使用公式: max(5, round(a * 24)) + (5 - (max(5, round(a * 24)) % 17)) % 17
                    # 其中 a*24 即为当前帧数
                    base = max(5, gen_frames)
                    target_frames = base + (5 - (base % 17)) % 17

                else:
                    target_frames = gen_frames
                ...
                if diff > 0:
                    if split_count == 1:
                        ...
                        if front_frames < diff:
                            raise ValueError(f"对齐失败 ({align_frames}): split_front 可用帧数 ({front_frames}) 不足以提供所需的 {diff} 帧。")
                        gen_start -= diff
                        front_end -= diff
                        ...
                        if back_frames < diff:
                            raise ValueError(f"对齐失败 ({align_frames}): split_back 可用帧数 ({back_frames}) 不足以提供所需的 {diff} 帧。")
                        gen_end += diff
                        back_start += diff
