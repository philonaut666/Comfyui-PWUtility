...
def load_audio_file(filepath: str) -> tuple[torch.Tensor, int]:
    ...
    with av.open(filepath) as af:
        if not af.streams.audio:
            raise ValueError("No audio stream found in the.")
...
class AudioLoaderPW:
    ...
    def load_audio(self, audio, start_time, end_time, duration, path=None, **kwargs):
        # If a path is connected from LocalMedia Manager, use it in priority
        ...
        try:
            if audio_to_load and audio_to_load != "none" and os.path.isabs(audio_to_load):
                ...
                # Copy absolute-path file into ComfyUI input dir so the frontend
                # can preview it via the standard /view endpoint (avoids cross-drive error)
                if os.path.exists(audio_path):
                    ...
                    # Use the relative filename for UI (standard /view endpoint)
