...
### align_frames
可将视频延长到视频模型要求的帧数。
支持none，MiniMax H3-17n+5(依据max(5, round(a * 24)) + (5 - (max(5, round(a * 24)) % 17)) % 17公式）, LTX2.3-8N+1(符合8n+1的也符合4n+1，就不再添加4n+1)。
...
## LMM Select Image PW， LMM Select audio PW, LMM Select Image PW， LMM Paths Extract PW
用于与[local media manager](https://github.com/Firetheft/ComfyUI_Local_Media_Manager)节点中获取图片，音频和视频。可多选可单选。并进行一定程度的调整。
当在Local Media Manager中多选，可以通过index选择具体某一个，也可以进行批量。
也可以选择直接接入图片、音频等，图片可提取图片中的prompt和seed值(如果有的话)。图片支持image list或者image batch,通过index获取其中的第几张图。
利用Comfyui-impact-pack节点中的Queue Trigger，用count控制index可实现依次读入的批量生成。
...
- [local media manager](https://github.com/Firetheft/ComfyUI_Local_Media_Manager)
